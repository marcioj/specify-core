...
module.exports =  spec('λ run()', function(it, spec) {
  it('Should return a promise of each tests\'s result.', function() {
    ...
    return expect(run([t1, t2, t3]).then(map(verdict)))
             .to.become(['success', 'failure', 'ignored'])
  })
  ...
  spec('the reporter', function(it) {
    ...
    it('Should emit `result` when any test is added.', function() {
      ...
      return expect(p2.then(function(){ return a }))
               .to.become(['success', 'failure', 'ignored'])
    })
  })
})
